...
def _():
    ...
    return data_path, mo, np, process_single_node_data, tune_mpc_params
...
@app.cell
def _(G, R, form, l, np, tune_mpc_params):
    gamma_list = np.logspace(start = -2, stop = 1, num = 4)
    ...
    best_row = tune_mpc_params(
        ...
        plot_list = ['objective', 'total load shedding'])
    return
...
@app.cell
def _(np):
    Q_list = 2**np.arange(0,5)
    H_list = 24*np.array([1,2,3,7])
    return (Q_list,)
