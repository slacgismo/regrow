...
def _():
    ...
    from plot_utils import compute_partition, plot_solution, plot_timeseries, solution_heatmaps
...
def _(
    ...
    for _key, _val in _m.items():
        print(f"{_key}: {_val:.3f}" if np.isfinite(_val) else f"{_key}: {_val}")
    return
...
@app.cell(hide_code=True)
def _(add_abnormal_event, event_duration_input, event_start_input, mo, pd, tidx):
...
@app.cell(hide_code=True)
def _(R, add_abnormal_event, event_duration_input, event_start_input, form, l, pd, plot_timeseries, s, shortfall, tidx):
...
@app.cell(hide_code=True)
def _(add_abnormal_event, event_duration_input, event_start_input, form, one_shot_solution, pd, plot_solution, s, tidx):
    _event_end = str((pd.Timestamp(event_start_input.value) + pd.Timedelta(days=int(event_duration_input.value) - 1)).date())
    _event_dates = [event_start_input.value, _event_end] if add_abnormal_event.value else None
    plot_solution(
...
@app.cell(hide_code=True)
def _(add_abnormal_event, event_duration_input, event_start_input, form, mo, one_shot_solution, pd, solution_heatmaps, tidx):
    _event_end = str((pd.Timestamp(event_start_input.value) + pd.Timedelta(days=int(event_duration_input.value) - 1)).date())
    _event_dates = [event_start_input.value, _event_end] if add_abnormal_event.value else None
    mo.output.append(
        solution_heatmaps(
