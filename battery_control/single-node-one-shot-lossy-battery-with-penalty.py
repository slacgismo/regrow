...
def _(mo):
    ...
    solve_twice = mo.ui.switch(label='resolve to polish battery', value=True)
    mo.vstack([make_problem, add_abnormal_event, solve_twice])
    return add_abnormal_event, make_problem, solve_twice
...
def _(
    ...
    if show_batt_power_bounds.value:
        _ax[1].axhline(problem.param_dict['B'].value, color='red', ls='--', linewidth=0.5)
        _ax[1].axhline(-problem.param_dict['B'].value, color='red', ls='--', linewidth=0.5)
    ...
    if show_cap_contrained.value:
        _ax[2].plot(tidx[_s][_charged], problem.var_dict['g'].value[_s][_charged], ls='none', marker='.', color='blue')
        _ax[2].plot(tidx[_s][_discharged], problem.var_dict['g'].value[_s][_discharged], ls='none', marker='.', color='orange')
    ...
    if show_battery_priority.value:
        ...
        _ax[2].plot(tidx[_s], naive_bp['g'][_s], linewidth=0.75)
    ...
    if show_utility_priority.value:
        ...
        _ax[2].plot(tidx[_s], naive_up['g'][_s], linewidth=0.75)
