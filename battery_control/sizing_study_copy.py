...
def _(
    ...
    problem.param_dict['lambda'].value = 20

    results_total = pd.DataFrame(columns=np.r_[['battery size'], metrics])
    results_event = pd.DataFrame(columns=np.r_[['battery size'], metrics])

    Qs = np.linspace(1, 50, 21)
    for _ix, _Q in enumerate(mo.status.progress_bar(Qs)):
        if solve_twice.value:
            problem_zero.param_dict['alpha'].value = 1.25
            problem_zero.param_dict['beta'].value = 0.5
            problem_zero.param_dict['lambda'].value = 20
            problem_zero.param_dict['Q'].value = _Q
            problem_zero.param_dict['B'].value = 0.333 * _Q
            problem_zero.solve(verbose=False, solver='CLARABEL')
            problem.param_dict['set_y'].value = np.array(problem_zero.var_dict['b'].value >= 0, dtype=int)
        problem.param_dict['Q'].value = _Q
        problem.param_dict['B'].value = 0.333 * _Q
...
def _(metrics, plt, results_total):
    results_total.plot(x='battery size', y=metrics[:2], marker='.', color=['C0', 'C1'])
    results_total.plot(x='battery size', y=metrics[2:], marker='.', color=['C2', 'C5'], ax=plt.gca(), ls='--')
    # plt.xscale('log')
...
def _(metrics, plt, results_event):
    results_event.plot(x='battery size', y=metrics[:2], marker='.', color=['C0', 'C1'])
    results_event.plot(x='battery size', y=metrics[2:], marker='.', color=['C2', 'C5'], ax=plt.gca(), ls='--')
    # plt.yscale('log')
    plt.xlabel('battery size')
    plt.title('During stress event')
    plt.gcf()
    return


@app.cell
def _(plt, problem, problem_zero):
    plt.plot(problem_zero.var_dict['y'].value[:100])
    _ax = plt.gca().twinx()
    _ax.plot(problem.var_dict['y'].value[:100], c='C1')
    return


@app.cell
def _(plt, problem_zero):
    plt.plot(problem_zero.var_dict['b'].value[:100])
    return


@app.cell
def _(plt, problem_zero):
    plt.hist(problem_zero.var_dict['y'].value)
    plt.gcf()
    return


@app.cell
def _(np, problem_zero):
    np.average(problem_zero.var_dict['y'].value)
    return
