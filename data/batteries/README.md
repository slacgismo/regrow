...
- *Technology codes `NAB` and `PBB` passed through unlabelled.* The code table in
  `tech_and_applications.py` had drifted a revision behind the one in
  `build_unit_table.py`, carrying `NAS` and `PBA`, which appear nowhere in the
  data. Both notebooks now use the same table and the notebook reports any code
  it cannot label.
- *Enclosure code `CS` was unmapped.* Same cause, same fix; unknown enclosure
  codes are now reported rather than passing through silently.
- *Round-trip efficiency.* The efficiency blend was removed from
  `tech_and_applications.py`. EIA-860 does not report efficiency, so the blend
  rested on placeholder values; the efficiency parameter is set elsewhere in the
  project. `technology_efficiency_blend.csv` is no longer produced.
