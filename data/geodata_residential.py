"""Update residential building geodata files"""
...
os.makedirs("geodata/residential",exist_ok=True)
...
#
# Generate residential building loadshapes
#
...
floorareas = pd.read_csv("res_loadshapes/floorareas.csv",index_col=["county","building_type"])
for file in sorted(os.listdir("res_loadshapes")):
    result = None
    if file.startswith("G") and file.endswith(".csv.zip"):
        ...
        csv = os.path.join("geodata","residential",geocode+(".csv.zip" if COMPRESS else ".csv"))
        ...
        data = pd.read_csv(os.path.join("res_loadshapes",file),
        ...
        result.columns = ["residential_buildings[MW]"]
