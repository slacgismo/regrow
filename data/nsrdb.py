...
Shell:
    bash$ python3 -m nsrdb -y|--year=YEARS -p|-position=LAT,LON 
...
Python:
    bash$ python3
    >>> import nsrdb
    >>> data = nsrdb.getyears(YEARS,LAT,LON)
    >>> nsrdb.writeglm(data,GLMNAME,OBJECTNAME,CSVNAME)
...
For example:
    ...
    #include (python3 -m nsrdb -y=2010 -p=37.5,-122.2 -g=/dev/stdout)

This is equivalent to 

    #python -m nsrdb -y=2010 -p=37.5,-122.2 -g=/tmp/example.glm
...
The following command downloads only the CSV data for a location:
    ...
    bash$ python3 -m nsrdb -y=2014,2015 -p=45.62,-122.70 -c=test.csv

The following command downloads the CSV data and creates a GLM file with the data linked and weather object named:

    bash$ python3 -m nsrdb -y=2014,2015 -p=45.62,-122.70 -c=test.csv -n=test -g=test.glm
...
def getkeys(new=False):
    ...
    try:
        ...
        if new:
            return {}
        raise Exception("unable to get API key for NSRDB data - see `python3 -m nsrdb help` for detail on NSRDB access credentials")
    return keys
...
if __name__ == "__main__":
    ...
    for arg in sys.argv[1:]:
        ...
        if token in ["-h","--help","help"]:
            ...
            print(f"use `python3 -m nsrdb --apikey=<your-apikey>` to set your api key")
        elif token == "--apikey":
            if not getemail():
                error(f"you have not signed up yet, use `python3 -m {os.path.basename(sys.argv[0]).replace('.py','')} --signup=<your-email>` to sign up",1)
            key = getkey(email)
            addkey(value)
            if not value:
                print(f"key for {email} deleted, use `python3 -m {os.path.basename(sys.argv[0]).replace('.py','')} --apikey={key}` to restore it")
        elif token == "--whoami":
            if not getemail():
                error(f"you have not signed up yet, use `python3 -m {os.path.basename(sys.argv[0]).replace('.py','')} --signup=<your-email>` to sign up",1)
