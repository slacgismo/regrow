...
This script updates the files in the res_loadshapes folder.
"""
...
# Weather: https://oedi-data-lake.s3.amazonaws.com/nrel-pds-building-stock/end-use-load-profiles-for-us-building-stock/2021/resstock_amy2018_release_1/weather/amy2018/G0100010_2018.csv
# Load: https://oedi-data-lake.s3.amazonaws.com/nrel-pds-building-stock/end-use-load-profiles-for-us-building-stock/2021/resstock_amy2018_release_1/timeseries_aggregates/by_county/state=CA/g0600010-mobile_home.csv
resstock_server = "https://oedi-data-lake.s3.amazonaws.com/nrel-pds-building-stock/end-use-load-profiles-for-us-building-stock/2021/resstock_amy2018_release_1/timeseries_aggregates/by_county/state=CA"
resstock_data = "{repo}/state={usps}/g{fips}{puma}-{type}.csv"

os.makedirs("res_loadshapes",exist_ok=True)

with open("res_loadshapes/floorareas.csv","w") as fh:
    ...
    for state_usps,state_fips in [(y,z) for x,y,z in states.state_codes if x in config.state_list]:
        for n in range(10,99999,20):
            file = f"res_loadshapes/G{state_fips}{n:05d}.csv.zip"
            result = []
            for type in building_types:
                try:
                    url = resstock_data.format(repo=resstock_server,usps=state_usps,fips=state_fips,puma=f"{n:05d}",type=type)
