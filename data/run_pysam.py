...
def remove_trailing_commas(srw_file_path):
    """
    Removes trailing commas in srw file to be used in SAM application.
    """
    with open(srw_file_path, "r") as f:
        lines = f.readlines()
    with open(srw_file_path, "w") as f:
        for l in lines:
            # Strip all trailing commas
            f.write(l.rstrip(",\n") + "\n")
...
if __name__ == "__main__":
    ...
    for idx, row in metadata.iterrows(): 
        ...
        srw_file_path = os.path.join(data_path, "nsrdb_data",
        ...
        srw = csv_to_srw(nsrdb_file_path, geocode, state, year,
                         lat, long, altitude, tz)
        srw.to_csv(srw_file_path, index=False, header=False)
        remove_trailing_commas(srw_file_path)
        ...
        plt.title("Single Turbine Predicted Power Output")
        plt.ylabel("Power Output [kW]")
        plt.xlabel("Date")
