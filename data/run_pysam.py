...
def filter_uswtdb_metadata(uswtdb_df):
    ...
    """
    required_metadata = ["model", "capacity[MW]", "hub_height[m]",
    ...
    # Get most common wind turbine model
    common_tur_model = uswtdb_df.groupby(["model"]).size().idxmax()
    common_tur_metadata = uswtdb_df[uswtdb_df["model"]==common_tur_model]
...
if __name__ == "__main__":
    ...
    uswtdb_df = pd.read_csv("wt_generators_assigned.csv")
    uswtdb_elevations_df = pd.read_csv("wt_generators_assigned_elevations.csv")
    wind_turbine_specs = pd.read_csv("turbine_model_metadata.csv")
    uswtdb_df = pd.merge(uswtdb_df,
                         uswtdb_elevations_df[['longitude_gen', 
                                               'latitude_gen', 'elevation[m]']],
                         on=['longitude_gen', 'latitude_gen'])
    uswtdb_df = pd.merge(uswtdb_df, wind_turbine_specs,
                         left_on=['model','capacity[MW]', 'hub_height[m]',
                                  'rotor_diameter[m]'], 
                         right_on=['gentype','capacity[MW]','hub_height[m]', 
                                   'rotor_diameter[m]'], how='left')
    master_df = filter_uswtdb_metadata(uswtdb_df).drop_duplicates()
    master_df.to_csv("uswtdb_pysam_sim.csv", index=False)
    counts = master_df['geohash'].value_counts().sort_values(ascending=True)
    master_df = pd.merge(master_df, counts, on='geohash')
    master_df = master_df.sort_values(by='count')
    regrow_folder = "s3://regrow/pysam_wind_powerplants/"
    ...
    for idx, row in master_df.iterrows():
        ...
        lat = float(row['latitude_gen'])
        lon = float(row['longitude_gen'])
        county = row["county"][-2:]
        city = row['county'][:-3]
        ...
        if filename in ran_files_list:
            ...
            # Power outputs
            all_power_output.to_csv((data_path + "/"+
                                      "single_turbine_power_timeseries/" +
                                      filename + ".csv"),
                                    index=False)
            # CONUS weather outputs
            master_weather_df.to_csv((data_path + "/"+
