...
class TZ(tzinfo):
    def __init__(self,name,tzoffset,dst):
        self.name = name
        self.tzoffset = tzoffset
        self.dst = dst
    def utcoffset(self,dt):
        return timedelta(hours=self.tzoffset)
    def dst(self,dt):
        return timedelta(hours=self.dst)
    def tzname(self,dt):
        return f"{self.tzoffset+self.dst:+02.0f}:00"
    def __repr__(self):
        return f"TZ({self.name},{self.tzoffset},{self.dst})"
