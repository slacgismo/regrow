...
USPVDB = pd.read_csv(zipdata.open([x for x in zipdata.namelist() if x.endswith(".csv")][0],"r"),
    usecols = ["p_state","p_county","ylat","xlong","p_area","p_name","p_year","p_tech_pri","p_axis","p_azimuth","p_battery","p_cap_ac"],
    )
USPVDB.columns =  ["state","county","latitude","longitude","area[m^2]","name","year","gentype","axis[deg]","azimuth[deg]","battery","capacity[MW]"]
USPVDB.set_index(["state","county"],inplace=True)
USPVDB.sort_index(inplace=True)

USPVDB = USPVDB.join(COUNTIES,how="inner").reset_index().set_index("bus").sort_index()
USPVDB["county"] = [f"{x} {y}" for x,y in USPVDB[["county","state"]].values]
USPVDB.drop("state",inplace=True,axis=1)
USPVDB.to_csv("uspvdb.csv",index=True,header=True)
