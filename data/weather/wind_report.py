...
def __(get_location, plt, wind):
    ...
    plt.title('Wind Speed (2018-2022)')
    time_series = plt.gcf()
...
def __(data_view, plt, sns):
    ...
    plt.xlabel('Days')
    plt.ylabel('Hours')
    plt.title('Heat map of Wind Speed (2018-2022)')
    heat_map = plt.gcf()
...
def __(mo):
    mo.md(
        r"""
        ## Hourly Statistics
        (1) Max residual wind speed. (2) August integrals 1st half, 2nd half and overall.
        """
    )
    return
...
def __(hourly_residual, mo):
    ...
    mo.md(f"Max residual speed: {max_hourly:.2f} (m/s)")
    return max_hourly,
...
def __(hourly_integral, mo):
    mo.md(f"Overall speed integral of August: {hourly_integral:.2f} (C˚)")
    return
...
@app.cell
def __(analyze_baseline, nodes, np, os, pd, utils, wind):
    ...
    for node in nodes:
        residual = analyze_baseline(wind[node])
