...
def _(am_solving, np, problem):
    am_solving
    evals, evecs = np.linalg.eig(problem.var_dict['V'].value)
    return (evals,)
...
@app.cell
def _(np, problem):
    Vnew = np.imag(problem.var_dict['V'].value) + 1j * np.real(problem.var_dict['V'].value)
    np.allclose(Vnew, Vnew.conjugate().T)
    return (Vnew,)


@app.cell
def _(Vnew, np):
    np.isclose(Vnew, Vnew.conjugate().T)
    return


@app.cell
def _(problem):
    problem.var_dict['V'].value - (problem.var_dict['V'].value).conjugate().T
    return
