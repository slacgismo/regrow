...
{{
    ...
    variable_cost {variable_cost} $/h/MW;
    scarcity_cost {scarcity_cost} $/h/MW^2;
}}
"""
...
if __name__ == '__main__':
    ...
    with open("powerplants.glm","w") as fh:
        ...
        print("#set relax_naming_rules=TRUE",file=fh)
        for plant_id,plant_data in egrid.iterrows():
            if plant_data["name"] in names:
                print(f"WARNING [powerplant.py]: powerplant {plant_data['name']} already processed, skipping duplicate(s)",file=sys.stderr)
                continue
            names.add(plant_data["name"])
