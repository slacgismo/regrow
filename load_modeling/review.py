...
def _(loads_ui, pd):
    training = (
        pd.read_csv("../data/geodata/total.csv", index_col=[0], parse_dates=[0])
        / 1000
    ).round(3)
    print(training[loads_ui.value])
    weather = (
        pd.read_csv(
            "../data/geodata/temperature.csv", index_col=[0], parse_dates=[0]
        )
        * 9
        / 5
        + 32
...
@app.cell
def _(pd, timezone_ui):
    prediction = (
        pd.read_csv(
            "wecc240_loads.csv", index_col=[0], parse_dates=[0]
        ).sort_index()
        / 1000
    )
    prediction.index = prediction.index.tz_localize("UTC").tz_convert(
        timezone_ui.value
    )
    return (prediction,)
...
@app.cell
def _(errors, mo, pd, prediction):
    nodes = pd.read_csv("../data/nodes.csv", index_col=[0]).sort_index()
    _nodes = nodes.join(prediction)[["Bus  Name", "Bus  Number"]].dropna()
    _nodes["name"] = [
        f"{x['Bus  Name']} ({x['Bus  Number']} @ {n})"
        for n, x in _nodes.iterrows()
    ]
    _nodes = (
        _nodes.drop(["Bus  Name", "Bus  Number"], axis=1)
        .sort_values("name")
        .to_dict("index")
    )
    _options = {
        y["name"]: x for x, y in _nodes.items() if x in errors.index.values
    }
    loads_ui = mo.ui.dropdown(
        label="Node:", options=_options, value=list(_options)[0]
    )
    return (loads_ui,)
...
def _(graph_ui, loads_ui, prediction, px):
    graph_prediction = px.line(
        prediction.loc["2020-08", loads_ui.value]
        if graph_ui.value
        else prediction[loads_ui.value],
        labels={
