...
def __(
    ...
    #
    main_view = mo.ui.tabs({
        "Objects": table_view,
        "Properties": data_view,
        "Map": get_mapview(),
        "Timeseries" : mo.vstack([mo.hstack([tape_selection,tape_plot],justify='start'),tape_view]),
...
@app.cell
def __(err, get_file, glm, json, mo, px, sys):
    #
    # Map view
    #
    def load_map():
        if (
            get_file() is None
            or "data" not in get_file()
            or len(get_file()["data"]) == 0
        ):
            return None
        ...
        data = get_file()["data"]
        try:
            _hoverdata = json.loads(glm["globals"]["map_data"]["value"])
        except Exception as err:
            print("ERROR",err,": no map_data",file=sys.stderr)
            _hoverdata = None
        try:
            _zoom = int(glm["globals"]["map_zoom"]["value"])
        except Exception as err:
            print("ERROR",err,": no map_zoom",file=sys.stderr)
            _zoom = None
...
@app.cell
def __(glm, mo):
    if glm and "objects" in glm:
        tapes = dict([(f"""{x["parent"]} ({x["property"]})""",n) for n,x in glm["objects"].items() if "class" in x and x["class"] in ["player","recorder"]])
    else:
        tapes = []
            
    tape_plot = mo.ui.switch(label="plot")    
    tape_selection = mo.ui.dropdown(options = tapes)
    return tape_plot, tape_selection, tapes
...
def __():
    ...
    return asyncio, go, json, mo, np, os, pd, px, subprocess, sys
