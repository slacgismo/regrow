...
def _():
    ...
    def make_constants (M, G, sparse, eta_store, eta_discharge):
        constants = {
            'hor_l': None,
            'hor_R': None,
            'G': G,
        ...
        return constants
    return cp, np, os, pd, plt


@app.cell(hide_code=True)
def _(b_charge, charge, cp, np, param_eta_charge):
    def one_shot(l, R, G):
        T = R.shape[0]
        ...
        constraints = [
            ...
            u <= G,
            b <= B,
            q[1:] == param_eta_storage*q[:-1]+ -b/param_eta_discharge + b_charge*param_eta_charge,
            s <= l,
            b + r + u == l - s,
            B == 0.33*param_Q,
            q[0] == 1.0*param_Q,
        ]
        objective = 1/T*(param_lambda*cp.sum(s)+ #param_gamma*cp.sum(c)+
...
@app.cell
def _(os):
    [i for i in os.listdir('test_data') if 'sparse' in i]
    return
...
@app.cell
def _(all_eta, all_sparse, caps, defaultdict, np):
    def get_burned_mw():
        burned_mw = defaultdict(list)
        for sparse_val in all_sparse:
            for eta_val in all_eta:
                name_root = (f'test_data/sparse{int(sparse_val*1000)}'+
                    f'eta{int(eta_val*100)}_')
    ...
    # print(pd.DataFrame(both_qs, index = ['simultaneous charge by battery size']))
                chrg[arr_both[0,:],arr_both[1,:]]
                dischrg[arr_both[0,:],arr_both[1,:]]
                cancelled = np.minimum(chrg[arr_both[0,:],arr_both[1,:]], dischrg[arr_both[0,:],arr_both[1,:]])
                result = np.zeros(6)
                for ir, row in enumerate(list(arr_both[0,:])):
                    result[row] += cancelled[ir]
                for iq, qval in enumerate([0,2,4,8,16,32]):
                    burned_mw['eta']+=[eta_val]
                    burned_mw['sparse']+=[sparse_val]
                    burned_mw['q']+=[qval]
                    burned_mw['burned']+=[float(result[iq])]
        return burned_mw     

    burned_map = get_burned_mw()
    return (burned_map,)


@app.cell
def _(burned_map):
    del burned_map[False]
    return
...
@app.cell
def _(burned_map, pd):
    burn_df = pd.DataFrame(burned_map)
    return (burn_df,)


@app.cell(hide_code=True)
def _(all_eta, all_sparse, mo):
    eta_choose = mo.ui.dropdown(all_eta, value = .98,
                                label='eta for varying sparsity weight')
    sprs_choose = mo.ui.dropdown(all_sparse, value = .01, 
                                 label='weight for varying eta')

    mo.vstack([eta_choose])
    return eta_choose, sprs_choose


@app.cell
def _(burn_df, eta_choose, plt):
    for sprs in burn_df['sparse'].unique():
        fix_eta = burn_df [
            (burn_df['sparse']==sprs) &
            (burn_df['eta']== eta_choose.value)
            ].copy()
        factor = 1-eta_choose.value**2
        plt.plot(fix_eta['q'], fix_eta['burned'], label = f'L1 sparsity weight = {sprs}')
    plt.xlabel('Q')
    plt.title(f'Burned MWhs with eta {eta_choose.value}')
    plt.ylabel('Simultaneous MWhs "burned"')
    plt.legend()
    plt.show()
    return
...
@app.cell
def _(mo, sprs_choose):
    mo.vstack([sprs_choose])
    return


@app.cell
def _(burn_df, plt, sprs_choose):
    for eta in burn_df['eta'].unique():
        fix_sparse = burn_df [
            (burn_df['sparse']==sprs_choose.value) &
            (burn_df['eta']==eta)
            ].copy()
       # 1-eta**2
        plt.plot(fix_sparse['q'], fix_sparse['burned'], label = f'eta = {eta}')
    plt.xlabel('Q')
    plt.ylabel('Simultaneous MWhs "burned"')
    plt.title(f'Burned MWhs with sparsity weight {sprs_choose.value}')
    plt.legend()
    plt.show()
    return
...
@app.cell
def _(arr_both, chrg, drop_qs):
    for i, sim in enumerate(arr_both[0,:]==drop_qs.value):
        if sim:
            chrg[drop_qs.value,i]

    return


@app.cell
def _(arr_both, drop_qs):
    (arr_both[0,:]==drop_qs.value).shape
    return
